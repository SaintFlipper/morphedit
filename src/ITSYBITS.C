...
#define SETCAPTIONSIZE(h,i)   (UINT)SetProp(h,(LPCSTR)"ibSize",(HANDLE)i)
#define GETCAPTIONSIZE(h)     (UINT)GetProp(h,(LPCSTR)"ibSize")
#define FREECAPTIONSIZE(h)    RemoveProp(h,(LPCSTR)"ibSize")

#define SETMENUWASUPFLAG(h,i) (UINT)SetProp(h,(LPCSTR)"ibFlag",(HANDLE)i)
#define GETMENUWASUPFLAG(h)   (UINT)GetProp(h,(LPCSTR)"ibFlag")
#define FREEMENUWASUPFLAG(h)  RemoveProp(h,(LPCSTR)"ibFlag")
                              ...
                              BOOL fMax, BOOL fActive )
{
    ...
    if (fWin31)
    {
		...
		if ((lpsz = GlobalAllocPtr (GHND, ui + 2)) != (LPSTR)NULL)
        {
            UINT    nBkMode ;
            ...
            if (fVert)
            {
                ...
                // try and use "modern").
				lstrcpy (lf.lfFaceName, "Morpheus editor dialog font");
...
BOOL NEAR PASCAL DoMenu( HWND hWnd )
{
    ...
    if ((hDC = GetWindowDC (hWnd)) != (HDC)NULL)
    {
        // Invert the icon
        //
